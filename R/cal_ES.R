...
cal_ES <- function(
  ...
  exposure,
  ref,
  measure = c("MD", "Cohen", "Glass", "CL", "Hedge", "Cliff"),
  hetsk = FALSE){
  ...
  mean_exp <- aggregate(y, by = x, FUN = mean)
  var_exp  <- aggregate(y, by = x, FUN = var)
  ...
  if("Glass" %in% measure){   # assumption of equal variances
    sp2_ref     <- var_exp[x==ref, ]$group_var  # standard deviation of the reference population
